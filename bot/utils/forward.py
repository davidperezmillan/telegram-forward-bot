...
async def forward_media_to_target(context, media_type, file_id):
    try:
        if media_type == "photo":
            await context.bot.send_photo(chat_id=TARGET_CHAT_ID, photo=file_id, has_spoiler=True)
            logger.info(f"Imagen reenviada de forma anónima a {TARGET_CHAT_ID}")
        elif media_type == "video":
            await context.bot.send_video(chat_id=TARGET_CHAT_ID, video=file_id, has_spoiler=True)
